...
if(isset($_GET['vubla_url'])){
    define('VUBLA_API',$_GET['vubla_url']);
} else {
    define('VUBLA_API','http://api.vubla.com');  
}
function get_content() {

    if(ini_get('allow_url_fopen')) {
        $source = file_get_contents(VUBLA_API.'/bgsys/?file=default&host='.$_SERVER['HTTP_HOST']);
    } else {
        //Get content
        $ch = curl_init();
        curl_setopt ($ch, CURLOPT_URL, VUBLA_API.'/bgsys/?file=default&host='.$_SERVER['HTTP_HOST']);
