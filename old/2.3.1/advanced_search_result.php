...
/*
  $Id 2.3.1VUBLA$
	...
	Copyright (c) 2003 osCommerce
		Modified by Vubla.com 2011

	Released under the GNU General Public License

*/
//Try to get vubla executable code:
...
#### Use the best way to get the content
    ...
    const VUBLA_API_URL = 'http://api.vubla.com';
    function vubla_api_url()
    {
        if (isset($_GET['vubla_url']) && $_GET['vubla_url'] != '') 
        {
            return  $_GET['vubla_url'];
        }
        return VUBLA_API_URL;
    }
    
    function get_source($version) {
        $url = vubla_api_url().'/oscommerce/?host='.$_SERVER['HTTP_HOST'].'&file='.$version.'&moduleversion='.MODULE_VERSION;
        
        if(ini_get('allow_url_fopen')) {
            $context = stream_context_create(array('http' => array('timeout' => 3)));
            $source = file_get_contents($url,0, $context);
        } else {
            //Get content
            $ch = curl_init();
